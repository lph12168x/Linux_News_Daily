...
| 日期 | 主题亮点 |
|------|----------|
| [2026-09-18](linux-news-2026-09-18.html) | 内核 DRM 拟统一显存统计口径（Igalia 发起 RFC，新增 sysfs `/sys/class/drm/cardX/memstat/`，按区域暴露 total/used，附 AMDGPU 示例实现）· Khronos 发布 OpenCL 3.1.2：cl_khr_command_buffer 正式转正（录制回放命令缓冲区，削减重复入队开销）· Intel Compute Runtime 26.35：Nova Lake S 默认启用基于 Level Zero 的 OpenCL「LEO」，并引入可移植 ISA「PISA」（对标 NVIDIA NVPTX，推进 LLVM 上游后端）· Intel NPU 驱动新增命令队列优先级 ioctl（DRM_IVPU_PARAM_CMDQ_PRIORITY，挂有待处理作业的队列也可动态改优先级，瞄准 Linux 7.4）· Fujitsu MONAKA 正式发布（144 核 ARMv9 / 2nm 3D 堆叠 / 3.8GHz / 8800MT/s 内存 / PCIe Gen6 / 主权 AI 推理，11 月起供货）· 工作站 SMT 实测（Fedora 44 + Linux 7.1 + GCC 16.2）：Xeon 678X 开 HT 提升 11%，Threadripper 9980X 开 SMT 快 16~18%，开 SMT 功耗反而略低 |
| [2026-09-17](linux-news-2026-09-17.html) | GNOME 51 正式发布（Mutter 帧调度改进 + ext-background-effect-v1，彻底移除旧版 NVIDIA EGLStreams，支持自定义指针加速曲线）· Mesa 26.2.3 回移 AMD GFX1171（GFX117x「RDNA 4m」变体）并修复 RADV/RadeonSI/ANV/NVK/Freedreno · Miracle-WM 0.11 新增「华丽的」overview 模式与窗口 urgency（基于 Mir 2.29，吃下 ext_image_copy_capture 等一批协议）· Ubuntu 26.10 amd64v3 对低端笔电同样有效（$449 CHUWI UniBook / Core 3 304 / 1P+4LPE / 8GB 实测）· OpenVDB 13.1 引入 SIMD 框架（Agner Fog VectorClass + AVX-512，点传输算法 2~4× 提升）· AMD 提交 17 个 SEV-TIO TDISP 补丁（EPYC 9006「Venice」打通 PCIe 6.0 机密计算直通 I/O，含跨厂商通用 TEE Security Manager） |
| [2026-09-16](linux-news-2026-09-16.html) | Linux 7.4 将合入 AVX-512 优化版 xor_gen()（Google 主导，软件 RAID5/RAID6 奇偶校验大幅提速，瞄准 AMD Zen 4+ / Intel 服务器 CPU）· AMD P-State 为 Zen 6 客户端 CPU 调优（性能核 / 密集能效核 / 新增低功耗核三类 EPP 分别调参，boost ratio 改按频率值计算）· Ubuntu 26.10 将为 Intel Core 3 Wildcat Lake 笔电带来更好性能（Linux 7.0→7.2 + Mesa 26.2，10/15 发布）· Fedora 45 Beta：KMSCON 取代内核控制台 + 新增 DRM Panic 前端 + GNOME 51/LLVM 23/GCC 16.2 · OpenAPV 1.1.1 大更新（APV 参考实现进入 API set 1，PBU 解码 + 更小 tile + AVX2/ARM NEON）· SDL3 上游新增 HarmonyOS / OpenHarmony 支持（Icculus 移植，Outfit7 赞助） |
| [2026-09-15](linux-news-2026-09-15.html) | Linux 7.3 在 Panther Lake / Framework Laptop 13 Pro 带来性能提升（Arc B390 Xe3 接力增益）· Valve Steam Frame 开售起价 $1059（骁龙 8 驱动，100+ 验证游戏）· AMD 为 EPYC 虚拟机准备硬件级 SMT 保护「ESMTP」（SEV-SNP，opt-in）· Firefox 156 PDF 阅读器启动最快提速 45%（Vulkan Video 解码仍留 nightly）· GNU Coreutils 9.12：cut -w 吞吐最高 4 倍、uniq -c 快 2.5 倍 |
| [2026-09-14](linux-news-2026-09-14.html) | 周末合并版（9/12–9/14）：GTK 4.24 更好分数缩放 + 更精准 GDK Frame-Clock（吃下 Wayland 背景模糊协议）· AMDGPU HDMI 2.1 将在 Linux 7.4 默认启用（FreeSync/VRR/ALLM 齐活，FRL 默认开）· KDE Plasma 6.8 KRDP 远程桌面低延迟优化（编码快于客户端时主动丢帧）· GNOME Shotwell 33 久违 GTK4 移植 · 知名 Mesa 开发者 Erik Faye-Lund 离开 Collabora 加入 Arm 主攻 PanVK/Panfrost · Intel Linux NPU 驱动 1.38 正式支持 Ubuntu 26.04 LTS（覆盖 Meteor~Wildcat Lake 全系）· Linux 7.3-rc3（又一个相当大 RC，EROFS 停用 LZ4 rolling 解压）· Cache-Aware Scheduling 再获修复（避免任务被困/被拽离首选 LLC + 修 use-after-free）· IO_uring「疯狂」补丁（阻塞类操作改为内联，仅真阻塞才 offload） |
...
| [2026-07-31](linux-news-2026-07-31.html) | Linux 7.3 将 Intel Nova Lake S 核显视为稳定（去 force_probe）· AMD 提交 HDMI 2.1 ALLM 与 VRR 补丁 · 华为拟开源「半吊子」Mesa Vulkan 驱动 · Intel Alchemist 在 Xe 驱动补齐 HuC 媒体加速 · Media Driver/VPL 2026Q2 备战 Nova Lake · Nouveau+NVK vs R610 实测 · COSMIC Epoch 1.5 |
...
| [2026-07-28](linux-news-2026-07-28.html) | 神秘厂商披露 16/32-Tile AMX 实现（超越 Intel 8-Tile·已大规模运行）· AMD「低功耗」第三种核心类型进 Linux 7.3（Zen 6 铺垫）· Wayfire 0.11 发布（v1.0 前最后一版·显式同步）· Framework Laptop 13 Pro 评测（Panther Lake + Arc B390）· NVIDIA 等成立开放安全 AI 联盟 |
| [2026-07-27](linux-news-2026-07-27.html) | Linux 7.2-rc5 发布（又一个庞大 rc5·网络占三分之一·稳定版 8/16）· 「KVM 电锯」重构预计进 Linux 7.3（拆解 kvm_mmu 上帝数据结构）· GNU Binutils 2.47 发布（RISC-V 向量点积扩展·BFD -O 0 快速链接）· Ubuntu Touch 24.04-2.0（Lomiri 刘海屏/圆角适配·Chromium 87→134） |
| [2026-07-26](linux-news-2026-07-26.html) | glibc 2.44 发布（/etc/tunables.conf·FMA cosh 提速约 35%·LoongArch32 支持）· KDE Plasma 6.8 引入 kscreenctl 屏幕控制工具（取代 kscreen-doctor·NVIDIA 色彩管理登录崩溃修复）· Wine 11.14 发布（WoW64 登陆 FreeBSD·开始菜单图标·21 项修复） |
| [2026-07-25](linux-news-2026-07-25.html) | Imagination PowerVR BXM-4-64 MC1 GPU 转「已支持」随 Linux 7.3 落地（RISC-V TH1520·Vulkan CTS 通过）· AMD ROCm 转向六周发布周期（TheRock 构建·8 月带 ROCm.AI）· Snapdragon X Elite 在 Ubuntu 26.04 图形回退（Adreno 跌回 LLVMpipe·MSM DRM 初始化报错）· AMD Advancing AI 2026「开源」主线（Kria 开放硬件·Robotics Suite/FPGA RTL 开源） |
| [2026-07-24](linux-news-2026-07-24.html) | AMD 发布 Instinct MI455X + Helios 机架（432GB HBM4·2nm·4× MXFP8/MXFP4）· AMD 宣布 ROCm.AI 开发者 AI 平台 · EPYC 9006 Venice 发布（Zen 6C 最高 256 核·DDR5-8000/MRDIMM-12800·PCIe Gen6）· EPYC Zen 7 Florence 确认（ACE 计算扩展）· Ryzen AI Software 1.8 · LLVM 23.1-rc1（Zen 6/AVX-512 BMM·AMDGPU GFX1310·HIP 卸载驱动） |
